...
public class PaintableSurface : MonoBehaviour
{
    ...
    private float worldSpanU, worldSpanV;   // UV各軸の1辺ワールド寸法（楕円ブラシ＝世界で円・非一様スケール対応）
    ...
    // 塗り毎に呼ばれる RadiiUV 等での GetComponent を避けるため Awake でキャッシュ
...
#pragma warning restore CS0618
    ...
    public void Paint(RaycastHit hit, float radius, byte inkDensity, byte inkColorId = 0)
    {
        ...
        float worldRadiusSq = radius * radius;   // 3D距離判定はワールド空間で（非一様スケール対応）

        float avgScale = AvgScale();
        float localRadius = radius / Mathf.Max(avgScale, 0.0001f);
        float localRadiusSq = localRadius * localRadius;   // obj_osumi通知用に残す
...
#pragma warning restore CS0618
        ...
        PaintInternal(uv, uvRadii, hitLocal, hitNormalLocal, worldRadiusSq, inkDensity, inkColorId);
    }
    ...
    {
        ApplyBrush(uv, uvRadii, hitLocal, hitNormalLocal, worldRadiusSq,
                            ...
                            byte inkDensity, byte inkColorId, bool erase)
    {
        ...
        Matrix4x4 l2w = transform.localToWorldMatrix;   // 3D判定をワールド空間で（非一様スケール対応）
        int cu = Mathf.FloorToInt(uv.x * gridW);
        int cv = Mathf.FloorToInt(uv.y * gridH);
        // #3対策: U/V別に走査範囲を出してグリッドにクランプ（細い板＋大ブラシで走査数が暴走するのを防ぐ）
        int crU = Mathf.Min(gridW, Mathf.CeilToInt(rU * gridW));
        int crV = Mathf.Min(gridH, Mathf.CeilToInt(rV * gridH));
        ...
        for (int dv = -crV; dv <= crV; dv++)
        {
            for (int du = -crU; du <= crU; du++)
            {
                int gx = cu + du;
                int gy = cv + dv;
                ...
                // 3D距離はワールド空間で（非一様スケールでも世界で真円・UV島の染み出し防止）
                float dist3DSq = l2w.MultiplyVector(cellPositions[idx] - hitLocal).sqrMagnitude;
                if (dist3DSq > worldRadiusSq) continue;
    ...
    /// <summary>
    /// 世界半径を UV 各軸の半径(rU, rV)に変換。UVテーブルから測った各軸の「ワールド寸法」(worldSpanU/V)で割るので、
    /// 非一様スケールで縦長にした板でも「世界で真円」になる（単一半径だとUV円→世界で楕円だった）。
    /// </summary>
    private Vector2 RadiiUV(float worldRadius)
    {
        float rU = (worldSpanU > 1e-4f) ? worldRadius / worldSpanU : 0.1f;
        float rV = (worldSpanV > 1e-4f) ? worldRadius / worldSpanV : 0.1f;
    ...
    public void Erase(RaycastHit hit, float radius)
    {
        ...
        EraseInternal(uv, uvRadii, hitLocal, worldRadiusSq);
    }
    ...
    public void EraseAt(Vector3 worldCenter, float radius)
    {
        ...
        for (int i = 0; i < cellValid.Length; i++)
        {
            ...
            float dist3DSq = l2w.MultiplyVector(cellPositions[i] - localCenter).sqrMagnitude;
            if (dist3DSq > worldRadiusSq) continue;
    ...
    private void EraseInternal(Vector2 uv, Vector2 uvRadii,
                               Vector3 hitLocal, float worldRadiusSq)
    {
        ApplyBrush(uv, uvRadii, hitLocal, Vector3.zero, worldRadiusSq,
                   0, 0, erase: true);
    }
